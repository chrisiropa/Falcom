...
BEGIN
   ...
   (
      ...
      PosKranX_Min_mm int NULL,
      PosKranX_Max_mm int NULL,
      PosKatzeY_Min_mm int NULL,
      PosKatzeY_Max_mm int NULL,
...
IF COL_LENGTH(N'dbo.FALCOM_KRAN_POSITION', N'X_mm') IS NULL ALTER TABLE dbo.FALCOM_KRAN_POSITION ADD X_mm int NULL;
IF COL_LENGTH(N'dbo.FALCOM_KRAN_POSITION', N'Y_mm') IS NULL ALTER TABLE dbo.FALCOM_KRAN_POSITION ADD Y_mm int NULL;
...
IF COL_LENGTH(N'dbo.FALCOM_KRAN_POSITION', N'AnfahrX_mm') IS NULL ALTER TABLE dbo.FALCOM_KRAN_POSITION ADD AnfahrX_mm int NULL;
IF COL_LENGTH(N'dbo.FALCOM_KRAN_POSITION', N'AnfahrY_mm') IS NULL ALTER TABLE dbo.FALCOM_KRAN_POSITION ADD AnfahrY_mm int NULL;
...
BEGIN
   ...
   (Name, Breite_mm, Laenge_mm, PosKranX_Min_mm, PosKranX_Max_mm, PosKatzeY_Min_mm, PosKatzeY_Max_mm, PosHubZ_Oben_mm, PosHubZ_Unten_mm, Aktiv, Bemerkung)
   VALUES
...
BEGIN
   UPDATE dbo.FALCOM_KRAN_LAYOUT
      SET Breite_mm = 24000,
          Laenge_mm = 24000,
          PosKranX_Min_mm = 1000,
          PosKranX_Max_mm = 24000,
          PosKatzeY_Min_mm = 500,
          PosKatzeY_Max_mm = 23500,
          PosHubZ_Oben_mm = 200,
...
WHEN MATCHED THEN
   UPDATE SET
      ...
      AnfahrX_mm = source.X_mm + source.Breite_mm / 2,
      AnfahrY_mm = source.Y_mm + source.Laenge_mm / 2,
      AnfahrZ_mm = @untenZ
WHEN NOT MATCHED THEN
   INSERT (ID, PositionsTyp, PositionsNr, Bezeichnung, Art, Bemerkung, X_mm, Y_mm, Breite_mm, Laenge_mm, AnfahrX_mm, AnfahrY_mm, AnfahrZ_mm)
   VALUES (source.ID, source.PositionsTyp, source.PositionsNr, source.Bezeichnung, source.Art, source.Bemerkung, source.X_mm, source.Y_mm, source.Breite_mm, source.Laenge_mm, source.X_mm + source.Breite_mm / 2, source.Y_mm + source.Laenge_mm / 2, @untenZ);
